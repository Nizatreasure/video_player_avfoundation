...
@property(readonly, nonatomic) AVPictureInPictureController *pipController;
...
- (void) pictureInPictureControllerWillStartPictureInPicture:(AVPictureInPictureController *)pictureInPictureController{
  if (_eventSink != nil) {
      _eventSink(@{@"event" : @"pip will start"});
    }
}

- (void) pictureInPictureControllerDidStartPictureInPicture:(AVPictureInPictureController *)pictureInPictureController{
  if (_eventSink != nil ) {
      _eventSink(@{@"event" : @"pip did start"});
    }
}
...
if (_eventSink != nil) {
      _eventSink(@{@"event" : @"pip failed to start"});
    }
}

- (void)pictureInPictureControllerWillStopPictureInPicture:(AVPictureInPictureController *)pictureInPictureController{
  if (_eventSink != nil) {
      _eventSink(@{@"event" : @"pip will stop"});
    }
}

- (void)pictureInPictureControllerDidStopPictureInPicture:(AVPictureInPictureController *)pictureInPictureController{
   if (_eventSink != nil) {
      _eventSink(@{@"event" : @"pip did stop"});
    }
}
...
if (_pipController.isPictureInPictureActive) {
 ...
 if (_eventSink != nil) {
      _eventSink(@{@"event" : @"pip not supported"});
    }
 }
}
...
-(void) setPip:(AVPlayerLayer *) playerLayer;
@end
...
- (void)togglePictureInPictureMode:(FLTPipModeMessage *)input error:(FlutterError **)error{   
    FLTVideoPlayer *player = self.playersByTextureId[input.textureId];
    player.isPip = input.isPip.boolValue;
    [player setPip];    
}
