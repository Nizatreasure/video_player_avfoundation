...
- (NSDictionary *)toMap;
@end
@interface FLTPipModeMessage ()
+ (FLTPipModeMessage *)fromMap:(NSDictionary *)dict;
- (NSDictionary *)toMap;
@end
...
@implementation FLTPipModeMessage
+ (instancetype)makeWithTextureId:(NSNumber *)textureId isPip:(NSNumber *)isPip {
  FLTPipModeMessage *pigeonResult = [[FLTPipModeMessage alloc] init];
  pigeonResult.textureId = textureId;
  pigeonResult.isPip = isPip;
  return pigeonResult;
}
+ (FLTPipModeMessage *)fromMap:(NSDictionary *)dict {
  FLTPipModeMessage *pigeonResult = [[FLTPipModeMessage alloc] init];
  pigeonResult.textureId = GetNullableObject(dict, @"textureId");
  NSAssert(pigeonResult.textureId != nil, @"");
  pigeonResult.isPip = GetNullableObject(dict, @"isPip");
  NSAssert(pigeonResult.isPip != nil, @"");
  return pigeonResult;
}
- (NSDictionary *)toMap {
  return [NSDictionary
      dictionaryWithObjectsAndKeys:(self.textureId ? self.textureId : [NSNull null]), @"textureId",
                                   (self.isPip ? self.isPip : [NSNull null]), @"isPip",
                                   nil];
}
@end
...
- (nullable id)readValueOfType:(UInt8)type {
  switch (type) {
    ...
    case 130:
      return [FLTPipModeMessage fromMap:[self readValue]];
...
- (void)writeValue:(id)value {
  if ([value isKindOfClass:[FLTCreateMessage class]]) {
    ...
    [self writeByte:129];
    [self writeValue:[value toMap]];
  } else if ([value isKindOfClass:[FLTPipModeMessage class]]) {
    [self writeByte:130];
    [self writeValue:[value toMap]];
...
void FLTAVFoundationVideoPlayerApiSetup(id<FlutterBinaryMessenger> binaryMessenger,
  ...
  }
  {
   ...
   {
    FlutterBasicMessageChannel *channel = [[FlutterBasicMessageChannel alloc]
           initWithName:@"dev.flutter.pigeon.AVFoundationVideoPlayerApi.togglePictureInPictureMode"
        binaryMessenger:binaryMessenger
                  codec:FLTAVFoundationVideoPlayerApiGetCodec()];
    if (api) {
      NSCAssert(
          [api respondsToSelector:@selector(togglePictureInPictureMode:error:)],
          @"FLTAVFoundationVideoPlayerApi api (%@) doesn't respond to @selector(togglePictureInPictureMode:error:)",
          api);
      [channel setMessageHandler:^(id _Nullable message, FlutterReply callback) {
        NSArray *args = message;
        FLTPipModeMessage *arg_msg = GetNullableObjectAtIndex(args, 0);
        FlutterError *error;
        [api togglePictureInPictureMode:arg_msg error:&error];
        callback(wrapResult(nil, error));
      }];
    } else {
      [channel setMessageHandler:nil];
